...
const ServicePulse: React.FC<ServicePulseProps> = ({ d, delay = 0, duration = 4 }) => (
  <g>
    <motion.g
      initial={{ opacity: 0 }}
...
const ServicesSection = () => {
  ...
  return (
    <section ref={sectionRef} className="relative w-full bg-[#fcfcfc] py-12 md:py-24 px-4 sm:px-6 md:px-10 overflow-hidden font-urbanist">
      ...
      <svg width="0" height="0" className="absolute invisible">
        <defs>
          <linearGradient id="service-pulse-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
      ...
      <div className="relative z-10 max-w-6xl mx-auto flex flex-col items-center">
        ...
        >
          <button
            ...
            className={`relative px-8 py-2.5 rounded-full text-sm font-semibold transition-all z-10 ${
              activeTab === "services" ? "bg-[#0A0A0A] text-white shadow-lg" : "text-gray-500"
            }`}
          >
            Our Services
          </button>
          <button
            ...
            className={`relative px-8 py-2.5 rounded-full text-sm font-semibold transition-all z-10 ${
              activeTab === "industries" ? "bg-[#0A0A0A] text-white shadow-lg" : "text-gray-500"
            }`}
          >
            Industries We Serve
          </button>
          ...
          <div className="w-full flex flex-col gap-8">
            ...
            <div className="relative w-full rounded-[32px] overflow-hidden bg-[#0A0A0A] aspect-[4/5] sm:aspect-video shadow-2xl">
              ...
              <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-50 z-10">
                {[100, 250, 400, 550, 700, 850].map((x, i) => (
                  <ServicePulse key={`mv-${i}`} d={`M ${x} 0 V 1000`} delay={i * 0.4} duration={2.5} />
                ))}
                {[100, 250, 400, 550].map((y, i) => (
                  <ServicePulse key={`mh-${i}`} d={`M 0 ${y} H 1500`} delay={i * 0.6} duration={2.5} />
                ))}
              </svg>

              <div className="absolute top-8 left-0 right-0 z-30 overflow-hidden py-2 px-2">
                <div className="flex whitespace-nowrap animate-marquee">
                  {[...marqueeItems, ...marqueeItems, ...marqueeItems].map((item, i) => (
                    <div
                      key={`m-${i}`}
                      className={`flex items-center gap-2 px-5 py-2 mx-2 rounded-full shadow-lg border transition-all
                       
                          ? "bg-white border-gray-100 text-gray-900"
                         
                      }`}
                    >
                      <div className={activeTab === "services" ? "text-blue-600" : "text-white"}>
                        {React.createElement(item.icon, { size: 14 })}
                      </div>
                      <span className="text-[13px] font-bold whitespace-nowrap uppercase tracking-wide">
                        {item.name}
                      </span>
                    </div>
                  ))}
            ...
            <div className="flex flex-col gap-6 px-2 items-end">
              <p className="text-gray-700 text-[15px] leading-relaxed">
                We harness modern technologies to create smart, efficient, and scalable solutions. By blending innovation with practical execution, we build systems that enhance productivity and deliver meaningful insights.
              </p>
              <button className="w-[60%] px-4 py-4 bg-[#056DBC] text-white rounded-full font-semibold  text-[15px] shadow-[0_4px_25px_rgba(5,109,188,0.3)]">
        ...
        <div className="hidden lg:flex flex-col items-center w-full">
          ...
          >
            {activeTab === "services" ? "Empowering Every Sector with Intelligent Solutions" : "Revolutionizing Industries with Tailored AI Integration"}
          </motion.h2>
          ...
          <div className="grid grid-cols-12 gap-8 w-full">
            ...
            <motion.div
              ...
              className="col-span-5 relative rounded-4xl overflow-hidden bg-[#050505] shadow-2xl h-[450px]"
            >
              ...
              <div className="relative z-20 flex flex-col justify-end h-full p-12">
                <p className="text-[22px] text-white/90 leading-snug">
                  Try Our AI Solutions that Power Modern Businesses with Cutting-Edge Technology.
                </p>
              </div>
            </motion.div>
            ...
            >
              <div className="flex flex-col gap-4">
                <p className="text-white/90 text-lg leading-relaxed">
                  We deliver tailored technology solutions across diverse industries, helping businesses adapt, innovate, and grow in a rapidly evolving digital landscape.
                </p>
                <div>
                  <button className="px-6 py-3 rounded-full bg-[#0ea5e9] text-white font-medium hover:bg-[#0284c7] transition-colors">
              ...
              <div className="grid grid-cols-2 gap-3">
                {activeData.map((item, index) => (
                  ...
                  >
                    <item.icon size={16} className="text-[#0ea5e9] shrink-0" />
